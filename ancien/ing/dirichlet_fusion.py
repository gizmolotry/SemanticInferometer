...
class DirichletFusionConfig:
    ...
    # Slime mold / Physarum settings
    use_slime_mold: bool = False  # If True, use iterative Physarum algorithm
    slime_mold_iterations: int = 10
    slime_mold_decay: float = 0.1  # Path decay rate
    slime_mold_reinforce: float = 1.5  # Reinforcement factor
...
class SharedRKSBasis(nn.Module):
    ...
    def forward(self, X: torch.Tensor) -> torch.Tensor:
        ...
        omega_scaled = self.omega.to(X.device) / self._sigma
...
class DirichletFusion(nn.Module):
    ...
    7. Slime mold: Optional iterative Physarum algorithm
    ...
    def __init__(self, config: DirichletFusionConfig = None):
        ...
        self._provenance = {
            ...
            'use_slime_mold': config.use_slime_mold,
        ...
        # Slime mold network weights (initialized if used)
        self._slime_weights: Optional[torch.Tensor] = None
    ...
    def forward(
        ...
        # =====================================================================
        # STEP 5: Optional slime mold adaptive weighting
        # =====================================================================
        if self.config.use_slime_mold:
            print(f"[DirichletFusion] Running Physarum slime mold ({self.config.slime_mold_iterations} iterations)...")
            self._slime_weights = self._slime_mold_optimize(
                phi_per_kernel, 
                n_iterations=self.config.slime_mold_iterations
            )
            # Re-fuse with slime mold weights
            samples_per_kernel = self._apply_slime_weights(phi_per_kernel, samples_per_kernel)
        ...
        # Add slime mold weights if used
        if self._slime_weights is not None:
            result['slime_mold_weights'] = self._slime_weights
    ...
    def _slime_mold_optimize(
        self, 
        phi_per_kernel: Dict[str, torch.Tensor],
        n_iterations: int = 10,
    ) -> torch.Tensor:
        """
        Physarum polycephalum-inspired adaptive network optimization.
        ...
        decay = self.config.slime_mold_decay
        reinforce = self.config.slime_mold_reinforce
    ...
    def _apply_slime_weights(
        self,
        phi_per_kernel: Dict[str, torch.Tensor],
        samples_per_kernel: Dict[str, torch.Tensor],
    ) -> Dict[str, torch.Tensor]:
        """
        Apply slime mold weights to re-weight bot contributions per kernel.
        
        Instead of uniform Dirichlet, uses adaptive weights from Physarum optimization.
        """
        if self._slime_weights is None:
        ...
        for k_idx, k_name in enumerate(kernel_names):
            ...
            # Get slime weights for this kernel
            kernel_weights = self._slime_weights[k_idx]  # [B]
    ...
    def forward_vmo(
        ...
        Args:
            ...
            use_slime_mold: override config.use_slime_mold
        ...
        Returns:
            ...
            - 'slime_weights': [K_m, B] adaptive weights (if use_slime_mold)
            - 'provenance': metadata dict
        """
        N, B, H = cls_per_bot.shape
        device = cls_per_bot.device
        ...
        if use_slime_mold is None:
            use_slime_mold = self.config.use_slime_mold
        ...
        # Step 4: Optionally run slime mold to get adaptive kernel weights
        if use_slime_mold:
            print(f"  Running slime mold ({self.config.slime_mold_iterations} iterations)...")
            kernel_weights = self._slime_mold_adapt(
                phi_per_kernel, 
                n_iterations=self.config.slime_mold_iterations
            )
            self._slime_weights = kernel_weights
        else:
        ...
        for k_idx, (kernel_name, phi) in enumerate(phi_per_kernel.items()):
            ...
            # Weight by slime mold kernel weights
    ...
    def _decompose_vmo_variance(
        ...
        """
        Decompose variance into V, M, O components.
    ...
    def _slime_mold_adapt(
        self,
        phi_per_kernel: Dict[str, torch.Tensor],
        n_iterations: int = 10,
    ) -> torch.Tensor:
        """
        Iterative slime mold (Physarum) adaptation.
        ...
        print(f"    Slime mold: {active_paths}/{K_m * B} paths active")
        for k_idx, k_name in enumerate(kernel_names):
...
# =============================================================================
"""
The slime mold (Physarum polycephalum) finds optimal paths by:
1. Spreading uniformly (exploration)
2. Reinforcing paths that connect food sources efficiently
...
- The α sweep PROBES the semantic manifold like slime mold probing a maze
...
@dataclass 
class PhysarumNetworkAnalysis:
    """Full slime mold analysis results for a corpus."""
    corpus_name: str
    n_articles: int
    ...
    @classmethod
    def load(cls, path: Path) -> 'PhysarumNetworkAnalysis':
        """Load from JSON."""
        with open(path) as f:
            d = json.load(f)
...
class PhysarumAnalyzer:
    """
    Slime mold semantic network analyzer.
    ...
    def analyze(
        ...
        verbose: bool = True,
    ) -> PhysarumNetworkAnalysis:
        """
        Run full Physarum analysis on a corpus.
        ...
        Returns:
            PhysarumNetworkAnalysis with full crack/bond topology
        """
        import datetime
        ...
        result = PhysarumNetworkAnalysis(
...
def compare_physarum_results(
    real: PhysarumNetworkAnalysis,
    shuffled: PhysarumNetworkAnalysis = None,
    constant: PhysarumNetworkAnalysis = None,
    random: PhysarumNetworkAnalysis = None,
) -> Dict:
    """
    Compare Physarum analyses across control conditions.
    ...
    if ordering_satisfied:
        results['verdict'] = "PASS: Slime mold found real semantic structure"
    else:
...
def prepare_crack_heatmap_data(analysis: PhysarumNetworkAnalysis) -> Dict:
    """
...
def prepare_alpha_response_curves(analysis: PhysarumNetworkAnalysis) -> Dict:
    """
...
def prepare_network_graph_data(
    analysis: PhysarumNetworkAnalysis,
...
if __name__ == "__main__":
    print("=" * 60)
    print("Testing Dirichlet Fusion + Multi-Kernel + Slime Mold")
    print("=" * 60)
    ...
    # Test 4: Slime mold adaptive weighting
    print("\n--- Test 4: Slime Mold Adaptation ---")
    config_slime = DirichletFusionConfig(
        rks_dim=256,
        n_observers=10,
        alpha=1.0,
        multi_kernel=True,
        kernel_types=['rbf', 'laplacian', 'polynomial'],
        use_slime_mold=True,
        slime_mold_iterations=5,
    )
    fusion_slime = DirichletFusion(config_slime)
    
    output_slime = fusion_slime.forward_vmo(
        cls_per_bot,
        compute_variance_decomposition=True,
    )
    print(f"Kernel weights after slime mold: {output_slime['kernel_weights']}")
    print("[OK] Slime mold test passed")
    
    # Test 5: Physarum analysis
    print("\n--- Test 5: Physarum Analyzer ---")
    
    analyzer = PhysarumAnalyzer(
        fusion=fusion,
        alphas=[0.1, 1.0, 10.0],
    ...
    print("[OK] Physarum analyzer test passed")
