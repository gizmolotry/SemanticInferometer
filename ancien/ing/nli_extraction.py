...
class ExtractionConfig:
    ...
    # Output mode
    use_cls_tokens: bool = False      # False = logits_raw, True = cls_views
    extract_logits: bool = True       # Always extract logits (as sidecar if CLS mode)
    ...
    apply_pca_to_cls: bool = True     # PCA removal on CLS only (never logits)
...
class UnifiedNLIExtractor(nn.Module):
    ...
    def _encode_single(self, premise: str, hypothesis: str) -> Dict[str, torch.Tensor]:
        """
        Encode a single premise-hypothesis pair with Dual-Signal extraction.
        
        UPGRADE: Now extracts both:
        1. CLS: The specific 'summary' token (standard)
        2. MEAN: Attention-weighted average of all tokens (The "Juice")
        """
        ...
        # 1. Raw Logits (The Verdict)
        logits = outputs.logits.squeeze(0).cpu()
        
        # 2. Hidden States (The Geometry)
        # last_hidden: [1, seq_len, hidden_dim]
        last_hidden = outputs.hidden_states[-1]
        
        # --- Signal A: CLS Token (Legacy/Decision) ---
        cls_token = last_hidden[:, 0, :].squeeze(0).cpu()
        
        # --- Signal B: Mean Pooling (The "Juice") ---
        # We perform attention-weighted averaging to ignore padding
        ...
        result = {
            'logits': logits,
            'cls': cls_token,       # Keep for backward compat
            'mean_pool': mean_pooled # NEW: Richer geometry for Track 2
        }
    ...
    def extract_article(self, article_text: str) -> Dict[str, torch.Tensor]:
        ...
        Returns dict with:
        - 'logits_raw': [n_bots, 3] raw verdict logits
        - 'logits_flat': [24] flattened
        - 'logits_triplet': [24] contrast triplets (backward compat)
        - 'cls_per_bot': [n_bots, hidden] V-observer embeddings
        - 'cls_stacked': [n_bots * hidden] flattened (backward compat)
        - 'rep_kind': which representation type this is
        ...
        # Accumulate weighted results across paragraphs
        logits_acc = None
        cls_acc = None
        attentions_list = []
        ...
        result = {
            # Logits channel (NEVER kernelized)
            ...
            # CLS channel (may be fused via Dirichlet)
