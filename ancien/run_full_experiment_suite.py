...
Runs all experiments (Real + 3 Controls) Ã— (N seeds)
...
def run_single_corpus(
    ...
    print(f"\n{'='*80}")
    print(f"RUNNING: {corpus.upper()} (mode: {mode})")
    print(f"{'='*80}")
        ...
        for pattern in patterns:
            ...
            if source_file.exists():
                ...
                if source_file != dest_file:
                    ...
                    print(f"  Renamed: {pattern} â†’ observer_{seed}.pt")
                else:
    ...
    if missing:
        print(f"âš  Missing expected output files for {corpus}: {len(missing)}")
        for m in missing[:10]:
            print(f"  - {m}")
        if len(missing) > 10:
...
def run_nli_probe_for_corpus(
    ...
    if result.returncode != 0:
        print(f"âŒ PROBE FAILED: {corpus}")
        print(f"STDERR: {result.stderr}")
        return {
            "status": "failed",
            "corpus": corpus,
    ...
    print(f"âœ… PROBE COMPLETED: {corpus}")
...
def run_comparison(exp_dir: Path, seeds: List[int]) -> bool:
    ...
    if result.returncode != 0:
        print(f"âŒ Comparison failed: {result.stderr}")
        return False
...
def main():
    ...
    parser.add_argument(
        ...
        default="enhanced",
        help=(
            "Experiment mode to run (default: enhanced). "
            "(Modes are validated by run_experiments.py; this script does not hardcode them.)"
        )
    )
    ...
    if not probe_hyp_path.exists():
        print(f"âš ï¸  WARNING: Probe hypotheses not found: {probe_hyp_path}")
        print(f"âš ï¸  Probe will be DISABLED")
        probe_enabled = False
    elif not probe_script_path.exists():
        print(f"âš ï¸  WARNING: Probe script not found: {probe_script_path}")
        print(f"âš ï¸  Probe will be DISABLED")
        probe_enabled = False
    ...
    if args.resume:
        ...
        print(f"\n{'='*80}")
        print("ðŸ”„ RESUMING EXPERIMENT SUITE")
        print(f"{'='*80}")
    ...
    # ðŸ” CHECKPOINT DETECTION - Skip already completed corpora
    ...
    if args.resume:
        ...
        for corpus in corpora:
            ...
            if missing_seeds:
                ...
                print(f"    âœ… Completed: {completed_seeds}")
                print(f"    â³ Remaining: {missing_seeds}")
            else:
                completed_corpora.add(corpus)
                print(f"  {corpus}: âœ… COMPLETE (all {len(args.seeds)} seeds done)")
        
        print(f"{'='*80}")
        
        if completed_corpora == set(corpora):
            print("\nâœ… ALL CORPORA COMPLETE! Nothing to resume.")
    ...
    for corpus in corpora:
        ...
        if probe_enabled:
            try:
                ...
                if probe_result.get("status") != "success" and not args.probe_nonfatal:
                    results.append(result)
                    print("\nâŒ Probe failed and --probe-nonfatal is not set.")
                    print("Stopping experiment suite.")
                    break
                ...
                if not args.probe_nonfatal:
                    results.append(result)
                    print(f"\nâŒ Probe exception: {e}")
                    print("Stopping experiment suite.")
                    break
                else:
                    print(f"âš  Probe exception (nonfatal): {e}")
        ...
        print(f"\n{'='*80}")
        print(f"ðŸ“Š PROGRESS: {completed_so_far}/{total_remaining} corpora completed in this run")
        print(f"{'='*80}\n")
    ...
    if all_success:
        ...
        print("\nâš  Skipping comparison due to earlier failures.")
    ...
    for r in results:
        status_emoji = "âœ…" if r.get("status") == "success" else "âŒ"
        print(f"  {status_emoji} {r.get('corpus')}: {r.get('status')}")

        if "probe" in r:
            p = r["probe"]
            p_emoji = "âœ…" if p.get("status") == "success" else "âŒ"
