            ...
            try:
                ...
                if dirichlet_results is not None and "fused" in dirichlet_results:
                    ...
                    waterfall_ckpt.save_t2_kernel_projections(
                        ...
                        rbf_sigma=self.dirichlet_fusion.basis.sigma if hasattr(self.dirichlet_fusion, 'basis') else 1.0,
            ...
            try:
                ...
                if cls_per_bot_list and hasattr(self, 'dirichlet_fusion') and self.dirichlet_fusion is not None:
                    try:
                        ...
                        for i in range(min(cls_stacked.shape[0], 100)):  # Cap at 100 for efficiency
                            # Track 3 Gate: Skip walker on Fog regions (high variance = decoherent)
                            # NOTE: Fog is a Track 3 atmospheric state, NOT a walker state.
                            # When in fog, walker is effectively "broken" (W -> inf, no path)
                            if fog_mask[i]:
                                walker_outputs.append(torch.zeros(rks_dim, device=device))
                                walker_work_integrals.append(float('inf'))
                                walker_states.append("broken")  # Track 4 state = broken (not "fog")
                                continue
                            ...
                            walker_outputs.append(result["walker_output"])  # [D]
                        ...
                        if walker_outputs:
                            ...
                            # Pad if we capped earlier
                            if walker_t4.shape[0] < len(cls_per_bot_list):
                                pad_size = len(cls_per_bot_list) - walker_t4.shape[0]
                                walker_t4 = torch.cat([walker_t4, walker_t4[-1:].expand(pad_size, -1)], dim=0)
                                walker_work_integrals.extend([walker_work_integrals[-1]] * pad_size)
                                walker_states.extend([walker_states[-1]] * pad_size)

                            # Count states
                            state_counts = {"elastic": 0, "trapped": 0, "broken": 0, "fog": 0}
                            for s in walker_states:
                                state_counts[s] += 1
                            mean_work = np.mean(walker_work_integrals)
                            print(f"[Track 4] Walker: {walker_t4.shape}, W={mean_work:.3f}, "
                                  f"states={{E:{state_counts['elastic']}, T:{state_counts['trapped']}, B:{state_counts['broken']}, F:{state_counts['fog']}}}")
                    except Exception as e:
                        import traceback
