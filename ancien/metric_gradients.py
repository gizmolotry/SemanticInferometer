...
∇(cosine) is orthogonal to the vector itself.
...
class MetricGradientConfig:
    ...
    pooling: str = 'cls'  # 'cls', 'mean', 'last_token'
...
class MetricGradientExtractor:
        ...
        if anchor_name not in self._anchor_cache:
            ...
            with torch.no_grad():
                ...
                if self.config.pooling == 'cls':
                    ...
                    anchor_vec = output.last_hidden_state.mean(dim=1)
                else:
    ...
    def get_metric_gradient(
        ...
        if self.config.pooling == 'cls':
            ...
            article_vec = hidden_state.mean(dim=1)
        else:
        ...
        if self.config.pooling == 'cls':
            ...
            gradient = hidden_state.grad.mean(dim=1).detach()
        else:
...
# =============================================================================
# ALPHA-SWEEP GEOMETRIC STABILITY GRADIENTS (Track 3 - Modern System)
# =============================================================================
#
# This section implements the "Geometric Stability Gradient" - the derivative
# of the Gram matrix structure with respect to the Dirichlet concentration
# parameter alpha.
#
# Equation:
#     Tension(α) ≈ || Gram(α+Δ) - Gram(α) ||_F / Δα
#
# High Tension = Geometry is unstable (fractures easily with observer change)
# Low Tension  = Geometry is robust (consensus is strong across observers)
#
# This is DISTINCT from the semantic axis tension above:
# - Semantic Axis Tension: How articles align with hypothesis pairs
# - Geometric Stability: How the entire article manifold deforms under α-change
# =============================================================================

def compute_alpha_gradients(
    ...
    This measures how the Gram matrix (pairwise article similarity) changes
    as we vary the Dirichlet concentration parameter α.

    Parameters
    ----------
    sweep_results : Dict
        Output from run_alpha_sweep() containing 'wavelength' dictionary
        with pairwise Gram matrix energies between alpha steps.
    alphas : List[float]
        The list of alpha values used in the sweep, sorted.

    Returns
    -------
    Dict containing:
        - gradients: List[float] scalar gradients (dE/dα) between alpha steps
        - intervals: List[str] labels for the steps (e.g. "0.1-0.5")
        - mean_tension: float (global geometric instability score)
        - max_tension: float (peak instability)
        - stability_score: float (0..1 proxy for robustness, 1=stable)
        - critical_interval: str (interval where geometry breaks most)
        - phase_transitions: List[str] (intervals with above-threshold tension)
    """
    # Extract Wavelength Energies (||G1 - G2||_F from sweep)
    wavelengths = sweep_results.get("wavelength", {})

    if not wavelengths:
        # Fallback: try to compute from per-alpha Gram matrices
    ...
    for i in range(len(sorted_alphas) - 1):
        ...
        # Key format from run_alpha_sweep
        key = f"{a1}_to_{a2}"

        # Robust retrieval (check forward and reverse keys)
        data = wavelengths.get(key)
        if data is None:
            key_rev = f"{a2}_to_{a1}"
            data = wavelengths.get(key_rev)

        if data is None:
            # Try numeric key formats
            key_alt = f"alpha_{a1}_to_{a2}"
            data = wavelengths.get(key_alt)

        if data is None:
            continue

        # Extract energy (Frobenius norm of Gram difference)
    ...
    if not gradients:
        return {"warning": "Insufficient alpha steps for gradient calculation", "gradients": []}
    ...
    # Critical Phase Transition: Where does the geometry break most?
    max_idx = int(np.argmax(gradients_arr))
    critical_interval = intervals[max_idx]

    # Phase transitions: intervals with tension > mean + 1*std
    threshold = mean_tension + std_tension
    phase_transitions = [intervals[i] for i, g in enumerate(gradients) if g > threshold]

    # Stability Score: Inverse soft-decay of tension
    # 1.0 = perfectly rigid, 0.0 = total chaos
...
def _compute_gradients_from_gram(
    sweep_results: Dict[str, Any],
    alphas: List[float],
) -> Dict[str, Any]:
    """
    Fallback: compute gradients directly from per-alpha Gram matrices.

    Used when sweep_results doesn't have pre-computed wavelength data.
    """
    gradients = []
    intervals = []
    sorted_alphas = sorted(alphas)
    ...
    for i in range(len(sorted_alphas) - 1):
        ...
        # Try to get Gram matrices for each alpha
        key1 = f"alpha_{a1}"
        key2 = f"alpha_{a2}"

        gram1 = sweep_results.get(key1, {}).get("gram_matrix")
        gram2 = sweep_results.get(key2, {}).get("gram_matrix")

        if gram1 is None or gram2 is None:
            continue

        # Convert to numpy if needed
        if hasattr(gram1, 'cpu'):
            gram1 = gram1.cpu().numpy()
        if hasattr(gram2, 'cpu'):
            gram2 = gram2.cpu().numpy()
    ...
    if not gradients:
        return {"warning": "Could not compute gradients from Gram matrices", "gradients": []}
    ...
    return {
        ...
        "stability_score": stability_score,
        "n_steps": len(gradients),
        "method": "gram_matrix_fallback",
    }


@dataclass
class AlphaStabilityConfig:
    """Configuration for alpha-sweep stability analysis."""

    alphas: List[float] = field(default_factory=lambda: [0.1, 0.5, 1.0, 5.0, 20.0])
    n_dirichlet_samples: int = 50  # O-observers per alpha
    rks_output_dim: int = 2048
    crn_enabled: bool = True  # Common Random Numbers for rigor
    crn_seed: int = 12345
    device: str = "cuda"
...
class AlphaStabilityAnalyzer:
    """
    Analyze geometric stability across Dirichlet alpha values.

    This is the "Modern System" Track 3: measures how the article manifold
    deforms as we change the observer concentration parameter.

    Usage:
        analyzer = AlphaStabilityAnalyzer(config)
        results = analyzer.analyze_corpus(articles, cls_embeddings)
    """
    ...
    def __init__(self, config: AlphaStabilityConfig = None):
        ...
        print(f"[AlphaStability] Alphas: {self.config.alphas}")
    ...
    def compute_gram_matrix(
        ...
        """Compute Gram matrix (pairwise cosine similarities)."""
        if normalize:
    ...
    def run_alpha_sweep(
        self,
        cls_per_bot: torch.Tensor,  # [N, n_bots, hidden_dim]
    ) -> Dict[str, Any]:
        """
        Run the full alpha sweep on CLS embeddings.

        For each alpha, we:
        1. Sample Dirichlet weights
        2. Fuse bot embeddings with those weights
        3. Compute Gram matrix
        4. Compare to adjacent alpha's Gram matrix

        Args:
            cls_per_bot: Per-bot CLS embeddings [N_articles, n_bots, hidden]

        Returns:
            Dict with per-alpha results and wavelength (Gram differences)
        """
        N, n_bots, hidden = cls_per_bot.shape
        cls_per_bot = cls_per_bot.to(self.device)

        print(f"[AlphaStability] Running sweep on {N} articles, {n_bots} bots")

        # Set up CRN if enabled
        ...
        for alpha in self.config.alphas:
            ...
            weights = dirichlet.sample((self.config.n_dirichlet_samples,))  # [n_samples, n_bots]

            # Fuse: weighted combination of bot embeddings
            # cls_per_bot: [N, n_bots, hidden]
            # weights: [n_samples, n_bots]
            # We average over samples to get expected embedding

            # Expand for broadcasting: [1, n_samples, n_bots, 1] x [N, 1, n_bots, hidden]
            weights_exp = weights.unsqueeze(0).unsqueeze(-1)  # [1, n_samples, n_bots, 1]
            cls_exp = cls_per_bot.unsqueeze(1)  # [N, 1, n_bots, hidden]

            # Weighted sum over bots
            fused = (weights_exp * cls_exp).sum(dim=2)  # [N, n_samples, hidden]

            # Average over samples for expected embedding
            fused_mean = fused.mean(dim=1)  # [N, hidden]
            fused_std = fused.std(dim=1).mean().item()  # scalar
            ...
            if total_var > 0:
                eff_rank_90 = int((cumsum < 0.9 * total_var).sum().item()) + 1
            ...
            results[alpha_key] = {
                ...
                "curvature": {
                    ...
                    "top_eigenvalue": float(eigenvalues[0].item()) if len(eigenvalues) > 0 else 0,
                },
            }

        # Compute wavelength (Gram matrix differences between adjacent alphas)
        wavelengths = {}
        ...
        for i in range(len(sorted_alphas) - 1):
            ...
            gram1 = gram_matrices[a1]
            gram2 = gram_matrices[a2]

            diff = gram2 - gram1
            energy = float(torch.norm(diff, p='fro').item())

            # Relative energy (normalized by average Gram norm)
            avg_norm = (torch.norm(gram1, p='fro') + torch.norm(gram2, p='fro')) / 2
            relative_energy = energy / (avg_norm.item() + 1e-9)

            wavelengths[f"{a1}_to_{a2}"] = {
                "energy": energy,
    ...
    def analyze_corpus(
        ...
        """
        Full analysis: run sweep and compute gradients.

        Args:
            articles: List of article dicts (for provenance)
            cls_per_bot: [N, n_bots, hidden] tensor from UnifiedExtractor

        Returns:
            Complete stability analysis including metric gradients
        """
        print(f"\n{'='*60}")
        print("ALPHA STABILITY ANALYSIS (Track 3 - Modern System)")
        print(f"{'='*60}")

        # Run the sweep
        sweep_results = self.run_alpha_sweep(cls_per_bot)

        # Compute metric gradients
        metric_gradients = compute_alpha_gradients(sweep_results, self.config.alphas)

        # Summary
        print(f"\nStability Score: {metric_gradients.get('stability_score', 0):.4f}")
        print(f"Mean Tension: {metric_gradients.get('mean_tension', 0):.4f}")
        print(f"Critical Interval: {metric_gradients.get('critical_interval', 'N/A')}")

        if metric_gradients.get('phase_transitions'):
            print(f"Phase Transitions: {metric_gradients['phase_transitions']}")

        return {
            "sweep_results": sweep_results,
            "metric_gradients": metric_gradients,
...
__all__ = [
    # Semantic Axis Tension (Original)
    ...
    # Alpha-Sweep Geometric Stability (Modern System Track 3)
...
if __name__ == "__main__":
    print("=" * 60)
    print("Testing Metric Gradient Extraction")
    print("=" * 60)
    ...
    print(f"\nTest article: {article[:60]}...")
    
    # Get gradients
    print("\n--- Computing gradients ---")
    grad_victim = extractor.get_metric_gradient(article, 'victim')
    grad_aggressor = extractor.get_metric_gradient(article, 'aggressor')
    
    print(f"Gradient shapes: victim={grad_victim.shape}, aggressor={grad_aggressor.shape}")
    
    # Compute tension
    print("\n--- Computing tension ---")
    tension = extractor.compute_tension(article, 'victim', 'aggressor')
    print(f"Correlation: {tension['correlation']:.4f}")
    print(f"Angle: {tension['angle_degrees']:.1f}°")
    print(f"Interpretation: {tension['interpretation']}")
    
    # The critical test from Gemini
    print("\n--- CRITICAL TEST ---")
    print(f"Gradient correlation (victim vs aggressor): {tension['correlation']:.4f}")
    print("Expected (Gemini): -0.2 to 0.3")
    print("If > 0.7: Anisotropy problem (gradients collapsed)")
    print("If < 0.5: Metric learning is working!")
