...
from scripts.run_observer_route_reserved_anchor_probe import build_route_reserved_anchor_suite


def _write_payload(path: Path, *, offset: float = 0.0) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    cls = np.zeros((12, 3, 4), dtype=np.float32)
    base = np.asarray(
        [
            [0.0, 0.0, 0.0, 0.0],
            [0.3, 0.2, 0.0, 0.1],
    ...
    cls[:, 1, 2] += np.asarray([0.0, 0.4, 0.0, 0.5, 0.0, 0.4, 0.0, 0.3, 0.0, 0.2, 0.0, 0.1], dtype=np.float32)
    torch.save(
        {
            "cls_per_bot": cls,
            "rks_basis_state": {
...
def test_route_reserved_anchor_suite_keeps_anchors_disjoint_from_routes(tmp_path: Path) -> None:
    ...
    payload = build_route_reserved_anchor_suite(
        payload_paths=[real, control],
        output_dir=tmp_path / "out",
        anchor_counts=[2, 3],
        anchor_strategies=["random_complement", "near_route_shell"],
        ...
        article_cap=None,
        pair_dim_cap=3,
        max_route_endpoint_fraction=0.6,
        min_route_pairs=2,
        configs=[
            {
                "label": "fixture",
    ...
    assert payload["diagnostic_type"] == "observer_route_reserved_anchor_probe"
    assert payload["summary"]["failure_count"] == 0
    assert payload["summary"]["run_count"] == 8
    assert payload["summary"]["route_reserved_summaries"]
    assert all(
        summary["mean_route_anchor_overlap"] == 0.0
        for summary in payload["summary"]["route_reserved_summaries"]
    )
    ...
    assert claim_matrix["summary_type"] == "observer_route_reserved_anchor_claim_matrix"
