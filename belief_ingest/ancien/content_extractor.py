"""
Multi-strategy content extraction with GDELT metadata support.
...
Now preserves GDELT metadata (themes, locations, sentiment_score) from historical mode.
"""
...
class ContentExtractor:
    ...
    Preserves GDELT metadata when available.
    """
    ...
    async def extract(self, article_meta: Dict) -> Optional[Dict]:
        ...
        Preserves GDELT metadata if present.
        
        Args:
            article_meta: Metadata from RSS/GDELT (must include 'url')
        
        Returns:
            Complete article dict with content, or None if extraction failed
        """
        ...
        for strategy in strategies:
            try:
                ...
                if result and self._validate_content(result):
                    ...
                    # Preserve GDELT metadata
                    result = self._preserve_gdelt_metadata(result, article_meta)
    ...
    def _matches_topic(article: Dict) -> bool:
        """Check if article matches Gaza/Israel topic (if filtering enabled)."""
    ...
    @staticmethod
    def _preserve_gdelt_metadata(result: Dict, article_meta: Dict) -> Dict:
        """
        Preserve GDELT metadata (themes, locations, sentiment) from historical fetch.
        """
        # Copy GDELT-specific fields if present
        gdelt_fields = ['themes', 'locations', 'sentiment_score']
        
        for field in gdelt_fields:
            if field in article_meta and article_meta[field]:
                result[field] = article_meta[field]
        ...
        test_urls = [
            {
                ...
                'source_type': 'test',
                'themes': ['ARMED_CONFLICT', 'MIDDLE_EAST'],
