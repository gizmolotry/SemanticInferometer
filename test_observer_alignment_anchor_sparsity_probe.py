...
from scripts.run_observer_alignment_anchor_sparsity_probe import build_anchor_sparsity_suite


def _write_payload(path: Path, *, offset: float = 0.0) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    cls = np.zeros((8, 3, 4), dtype=np.float32)
    base = np.asarray(
        [
            [0.0, 0.0, 0.0, 0.0],
            [0.3, 0.2, 0.0, 0.1],
    ...
    cls[:, 1, 2] += np.asarray([0.0, 0.4, 0.0, 0.5, 0.0, 0.4, 0.0, 0.3], dtype=np.float32)
    torch.save(
        {
            "cls_per_bot": cls,
            "rks_basis_state": {
...
def test_anchor_sparsity_suite_writes_anchor_budget(tmp_path: Path) -> None:
    ...
    payload = build_anchor_sparsity_suite(
        payload_paths=[real, control],
        output_dir=tmp_path / "out",
        anchor_counts=[3, None],
    ...
    assert payload["diagnostic_type"] == "observer_alignment_anchor_sparsity_probe"
    assert payload["summary"]["failure_count"] == 0
    assert payload["summary"]["anchor_cell_count"] > 0
    assert payload["summary"]["anchor_summaries"]
    ...
    assert claim_matrix["summary_type"] == "observer_alignment_anchor_sparsity_claim_matrix"
